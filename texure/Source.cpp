...
float lz = 0, tz = 1, cz = 0, cz1 = 0, zangle = 0, yangle = 0, xangle = 0;
float t = 0, y = 1, play = 0, game_over = 1;
bool paused = false;
bool flag = true;
...
void load(int imgnum) {
    ...
    if (imgnum == 2) {
        ...
        data = stbi_load("gameover.png", &width, &height, &nrChannels, 0);
        check(data);
    }
}
...
void first_page() {
    ...
    renderBitmapString(-105, 37, (void*)font2, "Press space to start game");
    renderBitmapString(-105, 27, (void*)font2, "Press 'esc' to exit game");
    renderBitmapString(-105, 17, (void*)font2, "Press RIGHT KEY in Keyboard to move car right ");
    renderBitmapString(-105, 7, (void*)font2, "Press LEFT KEY in Keyboard to move car left");
    renderBitmapString(-105, -3, (void*)font2, "Press DOWN KEY in Keyboard to increase car speed");
    renderBitmapString(-105, -13, (void*)font2, "Press 'a' move camera left");
    renderBitmapString(-105, -23, (void*)font2, "Press 'd' move camera right");
    renderBitmapString(-105, -33, (void*)font2, "Press 'w' move camera up ");
    renderBitmapString(-105, -43, (void*)font2, "Press 's' move camera down");
    renderBitmapString(-105, -50, (void*)font2, "Click right or left of mouse to pause the game");
...
void draw_road()
{
    ...
    //------------------green road----------------//
    glBegin(GL_QUADS);
    glColor3f(0.133, 0.45, 0.133);       // green
    glVertex3f(50, -2, 300);
    glVertex3f(3, -2, 300);
    glVertex3f(3, -2, -10000);
    ...
    glVertex3f(3.2, -2, 200);
    glVertex3f(3, -2, 200);
    glVertex3f(3, -2, -10000);
    glVertex3f(3.2, -2, -10000);
    glEnd();
    glBegin(GL_QUADS);
    glColor3f(0.5, 0.5, 0.5);
    glVertex3f(-3, -2, 200);
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (play != 2)
    {
        ...
        if (key == 's' && (upy >= (.9 * cos(.1))))                  //move down
        {
            upy -= .9 * cos(.1);
        }
        if (key == 'w')                   //move up
